...
class Topic extends Forum {
    ...
    public function get_topics($cat_id, $page = 1) {
        ...
        $qry = 'SELECT p.reputation,p.post_id,p.omessage AS message, p.post_created, r.rid, u.id, u.name AS name, u.avatar, ' .
               ...
               't.topic_status ' . 'FROM ' . PREFIX . 'codo_topics AS t ' . 
               ...
               'WHERE t.cat_id=' . $cat_id . ' AND (' . ' t.topic_status=' . Forum::APPROVED . ' OR' . 
               ' t.topic_status=' . Forum::APPROVED_CLOSED . ' OR' . ' t.topic_status=' . Forum::STICKY . ' OR' . 
               ' t.topic_status=' . Forum::STICKY_CLOSED . ' OR' . ' t.topic_status=' . Forum::STICKY_ONLY_CATEGORY . ' OR' . 
               ' t.topic_status=' . Forum::STICKY_ONLY_CATEGORY_CLOSED . '  ) AND ' . $this->getViewTopicPermissionConditions() . 
    ...
    public function getTaggedTopics($tag, $from = 0) {
        ...
        // username , title, topic created, no of replies,
        $qry = 'SELECT p.reputation,p.post_id, p.omessage AS message, p.post_created, r.rid, u.id, u.name as name, 
               u.avatar, c.cat_img, c.cat_alias,' . 't.topic_id, t.uid, t.title, t.no_posts, t.no_views, 
               t.last_post_time, t.last_post_uid, ' . 't.last_post_name AS last_post_name, t.topic_created, 
               t.cat_id, last_post_id,t.topic_status ' . 'FROM codo_topics AS t 
               INNER JOIN codo_tags as g ON g.topic_id=t.topic_id ' . 
               ...
               ' . $this->getViewTopicPermissionConditions() . ' AND g.tag_name=' . $this->db->quote($tag) . 
    ...
    private function getLatestTopics($from = 0) {
        ...
        $qry = 'SELECT  p.reputation,p.post_id, p.omessage AS message, p.post_created, r.rid, u.id, ' .
